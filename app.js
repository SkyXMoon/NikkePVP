...
const UI_TEXTS = {
  zh: {
    ...
    filterCommon: "常",
    filterCommonSummary: "常用",
    filterRegionCN: "国",
    filterRegionCNSummary: "国服",
  ...
  en: {
    ...
    filterCommon: "PVP",
    filterCommonSummary: "PVP",
    filterRegionCN: "CN",
    filterRegionCNSummary: "CN",
...
function parseFileNamesFromOcrText(rawText) {
  console.log("[OCR] 过滤前结果", String(rawText || ""));
...
function preprocessOcrCanvasForText(canvas) {
  ...
  for (let index = 0; index < data.length; index += 4) {
    luminanceValues.push(0.299 * data[index] + 0.587 * data[index + 1] + 0.114 * data[index + 2]);
  }
  const sorted = [...luminanceValues].sort((a, b) => a - b);
  const percentileIndex = Math.min(sorted.length - 1, Math.floor(sorted.length * 0.72));
  const threshold = Math.max(OCR_MIN_TEXT_LUMINANCE_THRESHOLD, sorted[percentileIndex] || OCR_MIN_TEXT_LUMINANCE_THRESHOLD);
  for (let index = 0, pixelIndex = 0; index < data.length; index += 4, pixelIndex += 1) {
    const value = luminanceValues[pixelIndex] >= threshold ? 0 : 255;
    data[index] = value;
    data[index + 1] = value;
    data[index + 2] = value;
    data[index + 3] = 255;
...
function createHelpModal(options = {}) {
  ...
  const sections = isEnglishLanguage()
    ...
    },
    {
      title: "队伍槽位",
      items: [
        "点击任意队伍切换当前操作队伍，当前队伍会高亮显示。",
        "拖动角色头像可调整站位，移动端也支持拖动。",
        "空槽会显示 P1-P5；可在空槽底部填写“充”作为万能充能值。",
    ...
    },
    {
      title: "充能轴",
      items: [
        "选择队伍后，上方图表会显示双方关键充能帧、标准 RL 轴、换弹/转身、空枪、晕眩与特殊事件。",
        "鼠标在图表内移动时，会自动吸附最近关键帧并显示详情。",
        "点击图表关键点也可查看详情，适合移动端使用。",
        "总充能轴会汇总角色攻击、万能充能、红莲反击、豺狼链接、罗珊娜献祭等有效贡献。",
        "角色充能计算统一按“基础充能 × X hit × Y人”展示；命中目标会按 P1-P5 展示实际 hit。",
    ...
    },
    {
      ...
      items: [
        ...
        "罗珊娜 96F 消除链接后，豺狼/波莉共同受击效果不再生效。",
        "爱蜜莉雅的本体额外伤害只作用本体，第二发起额外增加 13.01% 蓄速。",
      ],
    },
    {
      title: "角色选择",
    ...
    },
    {
      title: "分享",
      items: [
        "点击右侧“分享”按钮，或点击队伍栏的“图”，即可生成当前队伍分享图。",
        "普通竞技场会分享充能轴、防守 VS 进攻结果和双方队伍头像。",
        "冠军/特殊竞技场会分享各队充能结果、队伍头像和对应充能轴。",
        "桌面端会优先将图片放入剪贴板；移动端会优先打开系统分享。",
        "如果浏览器不支持直接分享，会弹出生成好的图片，可长按保存或分享。",
      ],
    },
    {
      title: "页面工具",
      items: [
        "左上角菜单可打开侧边栏，查看更新日志、使用说明、关于与建议和当前版本。",
...
function normalizePaidArenaLineupSlot(slot = {}, mode = "c") {
  return {
    ...
    activeTeamKey: normalizeTeamKey(slot.activeTeamKey || slot.dataTeamKey || "attack"),
  };
}
...
function swapPaidArenaSides() {
  const normalizedMode = normalizePaidArenaMode(state.paidArenaMode);
  if (normalizedMode === "normal") return;
  const swapSideRows = (store) => {
    if (!store?.[normalizedMode]) return;
    const entry = store[normalizedMode];
    [entry.defense, entry.attack] = [entry.attack, entry.defense];
  };
  swapSideRows(state.paidArenaTeams);
  swapSideRows(state.paidArenaUniversalCharges);
  swapSideRows(state.paidArenaChargeSpeeds);
  swapSideRows(state.paidArenaRosannaSacrificeFrames);
  swapSideRows(state.paidArenaRedHoodPierceCounts);
  swapSideRows(state.paidArenaScarletCounterEnabled);
  swapSideRows(state.paidArenaJackalLinks);
  openSlotSettings = null;
  openRosannaSacrificeSettings = null;
  saveTeam();
  render();
...
function createPaidArenaDataSourceBar() {
  ...
  bar.querySelector("[data-paid-side-swap]")?.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();
    swapPaidArenaSides();
  });
...
function loadTeam() {
  try {
    ...
    if (raw) {
      ...
      normalizeJackalLinks();
      setActiveTeam(isPaidArenaModeActive() ? saved.activeTeamKey || "attack" : "defense");
      return;
    }
  ...
  return `
    <div class="report-round-match">
      ...
      <div class="report-round-teams">
        <button class="report-choice-card report-round-team team-attack" type="button" data-winner="attack" data-round-index="${rowIndex}" aria-label="${escapeHtml(getTeamLabel("attack"))}">
          <strong>${escapeHtml(getTeamLabel("attack"))}</strong>
          ${getReportPreviewRowMarkup({ ...row, label: "" }, "attack")}
        </button>
        <span class="report-round-vs">VS</span>
        <button class="report-choice-card report-round-team team-defense" type="button" data-winner="defense" data-round-index="${rowIndex}" aria-label="${escapeHtml(getTeamLabel("defense"))}">
...
function updateSortSummary() {
  ...
  if (state.filters.common === "common") filters.push(ui.filterCommonSummary || ui.filterCommon);
  if (state.filters.region === "cn") filters.push(ui.filterRegionCNSummary || ui.filterRegionCN);
